...
def walkDir(root):
    for obj in sorted(os.listdir(root)):
        if (os.path.isdir(root + '/' + obj) and not (os.path.islink(root + '/' + obj)) and obj != "downloads"):
            walkDir(root + '/' + obj)
...
print
'%d files found' % (len(fileList))

leftSpace = size
currentChunk = []
for f in fileList:
    if f.size > size:
        print >> sys.stderr, 'ERROR: A single file (%s) is bigger (%d bytes) than the chunk size (%d bytes)' % (
        f.name, f.size, size)
        sys.exit(2)
    ...
    if f.size <= leftSpace:
        ...
        print
        'Chunk: %d files - %d bytes' % (len(currentChunk), (size - leftSpace))
...
print
'Exporting %d chunks to directory %s' % (len(chunks), exportDir)
